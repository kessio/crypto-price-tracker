    ...
    <div className="h-screen bg-gradient-to-b from-dark to-light">
        <div className="">
          ...
          <div className="bg-dark p-5 m-4 rounded flex justify-between">
            ...
            <span className="text-white font-lato font-bold text-2xl md:text-3xl">${Number(details.priceUsd).toLocaleString(undefined)}</span>
          </div>
          <div className="bg-dark p-5 m-4 rounded flex justify-between">
            <span className="text-white font-lato font-semibold md:text-3xl">Cap {details.marketCapUsd}</span>
          </div>
          <div className="bg-dark p-5 m-4 rounded font-lato font-semibold flex justify-between">
            <span className="text-white md:text-2xl">Vol {details.volumeUsd24Hr}</span>
          </div>
          <div className="bg-dark p-5 m-4 rounded font-lato font-semibold flex justify-between">
            <span className="text-white md:text-2xl">Supply {details.supply}</span>
          </div>
          <div className="bg-dark p-5 m-4 rounded font-lato font-semibold flex justify-between">
            <span className="text-white md:text-1xl"><a href={details.explorer} target="_blank" className="underline">See more</a></span>
          </div>
        </div>
    </div>
